    ...
    const checkAndLoadExamData = async () => {
      try {
        ...
        if (isExamGoal) {
          ...
          // Fallback 1: Match goal summary to exam_name
          if (!examIdToUse && goalData?.summary) {
            ...
            if (examNameMatch) {
              ...
              const { data: examMatch } = await supabase
                ...
                .limit(1)
                .maybeSingle()
              if (examMatch) examIdToUse = examMatch.id
            }
          }
          ...
          if (!examIdToUse) {
            ...
            if (examIdsWithDocs.length > 0) {
              const { data: examsData } = await supabase
                .from('course_exams')
                .select('id, exam_name, topics')
                .eq('user_id', user.id)
                .in('id', examIdsWithDocs)
              const taskTitleLower = task.title.toLowerCase()
              const taskWords = taskTitleLower.split(/\s+/).filter((w: string) => w.length > 2)
              let bestExam = null
              let bestScore = 0
              for (const ex of examsData || []) {
                let score = 0
                const examNameLower = (ex.exam_name || '').toLowerCase()
                const topicsLower = (ex.topics || []).map((t: string) => t.toLowerCase())
              ...
              else if (examIdsWithDocs.length === 1) examIdToUse = examIdsWithDocs[0]
            }
          }
          
          // Fallback 3: Most recent exam with documents
          if (!examIdToUse) {
            const { data: recentDoc } = await supabase
              ...
              .limit(1)
              .maybeSingle()
            if (recentDoc) examIdToUse = recentDoc.exam_id
          }
            ...
            setExamTopic(topic)
              ...
              try {
                ...
                if (!eid) {
                  ...
                  const hasEcon = /supply|demand|market|micro|economics/.test(taskLower)
                  const { data: m } = await supabase.from('exam_documents').select('exam_id').eq('user_id', user.id)
                    .ilike('document_name', hasEcon ? '%microeconomics%' : '%')
                    .limit(1).maybeSingle()
                  if (m?.exam_id) eid = m.exam_id
                  else {
                    const { data: a } = await supabase.from('exam_documents').select('exam_id').eq('user_id', user.id).limit(1).maybeSingle()
                    eid = a?.exam_id || null
                  }
                }
                if (eid) {
                  ...
                  setNeedsDocuments(true)
                  setExamNotes(task.notes || 'No documents found. Add study documents below.')
                }
              } catch (error) {
                console.error('Error loading notes:', error)
                setExamNotes(task.notes || 'Failed to load notes. Please try again.')
              } finally {
